...
{   
    ...
    ACalendarEventIndicator *eventIndicator;
    ...
    eventIndicator = [[ACalendarEventIndicator alloc] initWithFrame:CGRectZero];
...
- (void)layoutSubviews
{
    ...
    if (_subtitle) {
        ...
        _titleLabel.frame = CGRectMake(
                                       ...
                                       (self.contentView.A_height*5.0/6.0-height)*0.5+self.preferredTitleOffset.y,
                                       self.contentView.A_width,
                                       titleHeight
                                       );
        _subtitleLabel.frame = CGRectMake(
                                          self.preferredSubtitleOffset.x,
                                          (_titleLabel.A_bottom-self.preferredTitleOffset.y) - (_titleLabel.A_height-_titleLabel.font.pointSize)+self.preferredSubtitleOffset.y,
                                          self.contentView.A_width,
                                          subtitleHeight
                                          );
    } else {
        _titleLabel.frame = CGRectMake(
                                       ...
                                       self.contentView.A_width,
                                       floor(self.contentView.A_height*5.0/6.0)
                                       );
    }
    
    _imageView.frame = CGRectMake(self.preferredImageOffset.x, self.preferredImageOffset.y, self.contentView.A_width, self.contentView.A_height);
    
    CGFloat titleHeight = self.bounds.size.height*5.0/6.0;
    CGFloat diameter = MIN(self.bounds.size.height*5.0/6.0,self.bounds.size.width);
    diameter = diameter > ACalendarStandardCellDiameter ? (diameter - (diameter-ACalendarStandardCellDiameter)*0.5) : diameter;
...
#define kAnimationDuration ACalendarDefaultBounceAnimationDuration
...
- (UIColor *)colorForCurrentStateInDictionary:(NSDictionary *)dictionary
{
    if (self.isSelected) {
        if (self.dateIsToday) {
            return dictionary[@(ACalendarCellStateSelected|ACalendarCellStateToday)] ?: dictionary[@(ACalendarCellStateSelected)];
        }
        return dictionary[@(ACalendarCellStateSelected)];
    }
    if (self.dateIsToday && [[dictionary allKeys] containsObject:@(ACalendarCellStateToday)]) {
        return dictionary[@(ACalendarCellStateToday)];
    }
    if (self.placeholder && [[dictionary allKeys] containsObject:@(ACalendarCellStatePlaceholder)]) {
        return dictionary[@(ACalendarCellStatePlaceholder)];
    }
    if (self.weekend && [[dictionary allKeys] containsObject:@(ACalendarCellStateWeekend)]) {
        return dictionary[@(ACalendarCellStateWeekend)];
    }
    return dictionary[@(ACalendarCellStateNormal)];
}
...
@interface ACalendarEventIndicator ()
...
@implementation ACalendarEventIndicator
...
{
    [super layoutSubviews];
    CGFloat diameter = MIN(MIN(self.A_width, self.A_height),ACalendarMaximumEventDotDiameter);
    self.contentView.A_height = self.A_height;
    self.contentView.A_width = (self.numberOfEvents*2-1)*diameter;
    ...
    if (layer == self.layer) {
        ...
        for (int i = 0; i < self.eventLayers.count; i++) {
            ...
            if (!eventLayer.hidden) {
                eventLayer.frame = CGRectMake(2*i*diameter, (self.A_height-diameter)*0.5, diameter, diameter);
...
@implementation ACalendarBlankCell
