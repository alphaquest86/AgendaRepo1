...
@implementation ACalendar (Deprecated)

@dynamic identifier, lineHeightMultiplier;

- (void)setShowsPlaceholders:(BOOL)showsPlaceholders
{
    self.placeholderType = showsPlaceholders ? ACalendarPlaceholderTypeFillSixRows : ACalendarPlaceholderTypeNone;
}

- (BOOL)showsPlaceholders
{
    return self.placeholderType == ACalendarPlaceholderTypeFillSixRows;
}
...
- (NSDate *)tomorrowOfDate:(NSDate *)date
{
    ...
    components.day++;
    components.hour = ACalendarDefaultHourComponent;
    return [self.gregorian dateFromComponents:components];
}
...
{
    return [self isDate:date equalToDate:[NSDate date] toCalendarUnit:ACalendarUnitDay];
}
...
- (void)setIdentifier:(NSString *)identifier
{
    ...
    [self A_performSelector:NSSelectorFromString(@"invalidateDateTools") withObjects:nil, nil];
    
    if ([[self valueForKey:@"hasValidateVisibleLayout"] boolValue]) {
        [self reloadData];
    }
    [self A_setVariable:[self.gregorian dateBySettingHour:0 minute:0 second:0 ofDate:self.minimumDate options:0] forKey:@"_minimumDate"];
    [self A_setVariable:[self.gregorian dateBySettingHour:0 minute:0 second:0 ofDate:self.currentPage options:0] forKey:@"_currentPage"];
    [self A_performSelector:NSSelectorFromString(@"scrollToPageForDate:animated") withObjects:self.today, @NO, nil];
}
...
{
    self.rowHeight = ACalendarStandardRowHeight*MAX(1, ACalendarDeviceIsIPad*1.5);
}
