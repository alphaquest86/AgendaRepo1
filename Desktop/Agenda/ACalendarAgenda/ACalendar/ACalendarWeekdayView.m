...
@interface ACalendarWeekdayView()
...
@implementation ACalendarWeekdayView
...
- (void)layoutSubviews
{
    ...
    CGFloat contentWidth = self.contentView.A_width;
    ACalendarSliceCake(contentWidth, count, widths);
    
    CGFloat x = 0;
    for (NSInteger i = 0; i < count; i++) {
        CGFloat width = widths[i];
        UILabel *label = [self.weekdayPointers pointerAtIndex:i];
        label.frame = CGRectMake(x, 0, width, self.contentView.A_height);
        x += width;
    }
    free(widths);
}
...
{
    BOOL useVeryShortWeekdaySymbols = (self.calendar.appearance.caseOptions & (15<<4) ) == ACalendarCaseOptionsWeekdayUsesSingleUpperCase;
    ...
    BOOL useDefaultWeekdayCase = (self.calendar.appearance.caseOptions & (15<<4) ) == ACalendarCaseOptionsWeekdayUsesDefaultCase;
